<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TmpInfo-PWAds.aspx.cs" Inherits="ECommerce.Web.Manage.Template.TmpInfo_PWAds" %>
...
<head runat="server">
    ...
    <script>
        ...
        $(".new-info").live("click", function () {
            window.top.$op = window;
            window.top.$tmp = $(this);
            window.top.$modal = window.top.$.scojs_modal({ remote: '/Manage/News/AddAdInfoPic.aspx?spid=<%=Request.QueryString["spid"]%>&tiid=<%=Request.QueryString["tiid"]%>&ptiid=<%=Request.QueryString["ptiid"]%>&itid=<%=Request.QueryString["itid"]%>&sortnum=' + $(this).attr("data-sortnum") + '&bgw=' + $(this).attr("data-bgw") + '&bgh=' + $(this).attr("data-bgh"), title: '新增图文广告' });
            window.top.$modal.show();
            return false;
        });
        <%--编辑块内容--%>
        $(".edit-info").live("click", function () {
            window.top.$op = window;
            window.top.$tmp = $(this);
            window.top.$modal = window.top.$.scojs_modal({ remote: '/Manage/News/AddAdInfoPic.aspx?spid=<%=Request.QueryString["spid"]%>&tiid=<%=Request.QueryString["tiid"]%>&ptiid=<%=Request.QueryString["ptiid"]%>&itid=<%=Request.QueryString["itid"]%>&sortnum=' + $(this).attr("data-sortnum") + '&bgw=' + $(this).attr("data-bgw") + '&bgh=' + $(this).attr("data-bgh") + '&iid=' + $(this).attr("data-iid"), title: '编辑图文广告' });
            window.top.$modal.show();
            return false;
        });
        <%--删除块内容--%>
            ...
            if (confirm("是否删除该模块内容？")) {
                $.ajax({
                    ...
                    success: function (data) {
                        var arr = data.split("|~|");
                        if (arr[0] == 0) {
                            window.location = "/Manage/News/RedircetAD.aspx?spid=<%=Request.QueryString["spid"]%>&tiid=<%=Request.QueryString["ptiid"]%>&itid=<%=Request.QueryString["itid"]%>";
                        } else {
                            alert(arr[1]);
                        }
                    }
        ...
        <%--新增推荐--%>
        $(".add-rec").live("click", function () {
            window.top.$op = window;
            window.top.$tmp = $(this);
            window.top.$modal = window.top.$.scojs_modal({ remote: '/Manage/News/SelRecInfo.aspx?spid=<%=Request.QueryString["spid"]%>&tiid=<%=Request.QueryString["tiid"]%>&ptiid=<%=Request.QueryString["ptiid"]%>&itid=<%=Request.QueryString["itid"]%>&iid=' + $(this).attr("data-iid") + '&sortnum=' + $(this).attr("data-sortnum") + '&bgw=' + $(this).attr("data-bgw") + '&bgh=' + $(this).attr("data-bgh"), title: '新增图文广告' });
            window.top.$modal.show();
            return false;
        });
        <%--删除推荐--%>
        $(".del-rec").live("click", function () {
            var aiid = $(this).attr("data-aiid");
            if (confirm("确认删除？")) {
                $.ajax({
                    type: 'POST',
                    url: '/Manage/News/Ajax_SelRecInfo.aspx?daiid=' + aiid,
                    success: function (data) {
                        var arr = data.split("|~|");
                        if (arr[0] == 0) {
                            window.location = "/Manage/News/RedircetAD.aspx?spid=<%=Request.QueryString["spid"]%>&tiid=<%=Request.QueryString["ptiid"]%>&itid=<%=Request.QueryString["itid"]%>";
    ...
    <script>
        ...
        $(".delpage").live("click", function () {
            if (confirm("你确定要删除该页模板吗？")) {
                $.ajax({
                    type: 'POST',
                    url: '/Manage/Template/Ajax_DelTempInfo.aspx?delad=<%=Request.QueryString["tiid"]%>',
                    success: function (data) {
                        var arr = data.split("|~|");
                        if (arr[0] == 0) {
                            window.location = "/Manage/News/RedircetAD.aspx?spid=<%=Request.QueryString["spid"]%>&tiid=<%=Request.QueryString["ptiid"]%>&itid=<%=Request.QueryString["itid"]%>";
    ...
    <form id="form1" runat="server">
        <div>
            ...
            <div class="wraper">
                ...
                <div class="newsbox" style="margin-left:150px;">
