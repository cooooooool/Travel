...
namespace ECommerce.Web.Manage.Adver
{
    public partial class Default : UI.WebPage
    {
        ...
        private void BindData(bool isFirstPage)
        {
            ...
            int pageSize = 10;
            //分页查询语句
            string sql = "select row_number() over(order by  oo.CreateDate desc) as rownum,oo.* FROM Advertisement oo where 1=1 ";
            var name = string.Empty;
            if (!string.IsNullOrEmpty(txtRealName.Value))
            {
                name = txtRealName.Value;
                sql += " and  oo.AName like '%" + name + "%' ";
            }
            ...
            //分页方法
            Pager1.GetDataBind("Repeater", "rptListWork", sql, pageNum, pageSize, "", "rownum", "Default.aspx?name=" + name + "&");
            #endregion
        }
