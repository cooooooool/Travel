<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ChooseTemplate.aspx.cs" Inherits="ECommerce.Web.Manage.Systems.ChooseTemplate" %>
        ...
        $(".sel-temp").live("click", function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var arr = data.split("|~|");
                    if (arr[0] == 0) {
                        window.location = "/Manage/Template/Redircet.aspx?itid=" + itid + "&pid=<%=Request.QueryString["pid"] %>&page=<%=Request.QueryString["page"] %>&spid=<%=Request.QueryString["spid"]%>&tiid=" + arr[1];
                    } else {
                        alert(arr[1]);
                    }
                }
