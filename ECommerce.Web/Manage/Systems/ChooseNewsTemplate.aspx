<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ChooseNewsTemplate.aspx.cs" Inherits="ECommerce.Web.Manage.Systems.ChooseNewsTemplate" %>
...
<head>
    ...
    <script>
        ...
        $(".sel-temp").live("click", function () {
            ...
            $.ajax({
                type: 'POST',
                url: '/Manage/Systems/Ajax_TempInfo/AddTempInfo.aspx?itid=' + itid + '&spid=<%=Request.QueryString["spid"] %>&pid=<%=Request.QueryString["pid"] %>&page=<%=Request.QueryString["page"] %>&tid=' + $("input[name='iptTemp']:checked").val(),
                success: function (data) {
                    window.top.$modal.destroy();
                    var arr = data.split("|~|");
                    if (arr[0] == 0) {
                        window.location = "/Manage/News/Redircet.aspx?itid=" + itid + "&pid=<%=Request.QueryString["pid"] %>&page=<%=Request.QueryString["page"] %>&spid=<%=Request.QueryString["spid"]%>&tiid=" + arr[1];
                    } else {
                        alert(arr[1]);
                    }
                }
...
<body>
    <form runat="server" id="form8">
        <div class="header clearfix">
            <div class="backup"><a href="/Manage/Template/Redircet.aspx?spid=<%=Request.QueryString["spid"]%>&itid=<%=Request.QueryString["itid"]%>" class="back_btn backfirst">返回首页</a><a href="/Manage/News/Redircet.aspx?spid=<%=Request.QueryString["spid"]%>&pid=<%=Request.QueryString["pid"] %>&page=<%=Request.QueryString["page"] %>&tiid=<%=Request.QueryString["tiid"]%>&itid=<%=Request.QueryString["itid"]%>" class="back_btn">返回上级</a> </div>
