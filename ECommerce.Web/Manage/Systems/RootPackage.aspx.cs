...
{
    public partial class RootPackage : UI.WebPage
    {
        ...
        private void BindData(bool isFirstPage)
        {
            ...
            int pageSize = 10;
            //分页查询语句
            string sql = "select row_number() over(order by  RPID DESC) as rownum,oo.* FROM RootPackage oo where 1=1 ";
            var name = string.Empty;
            if (!string.IsNullOrEmpty(txtRealName.Value))
            {
                name = txtRealName.Value;
                sql += " and  oo.RPName like '%" + name + "%' ";
            }
            ...
            //分页方法
            Pager1.GetDataBind("Repeater", "rptListWork", sql, pageNum, pageSize, "", "rownum", "RootPackage.aspx?AreaId=" + Request.QueryString["AreaId"] + "&name=" + name + "&");
            #endregion
        }
        ...
        protected string GetStatus(object eval, object rpid)
        {
            return "<a href=\"javascript:;\" class=\"btn btn-mini\" onclick=\"padmit('" + rpid + "');\">送审</a> <a href=\"javascript:void(0);\" class=\"btn btn-mini\" data-title=\"编辑资源包信息\" onclick=\"editData('" + rpid + "')\">编辑</a>";
            if ("0" == eval.ToString())
            {
                return "<a href=\"javascript:;\" class=\"btn btn-mini\" onclick=\"padmit('" + rpid + "');\">送审</a> <a href=\"javascript:void(0);\" class=\"btn btn-mini\" data-title=\"编辑资源包信息\" onclick=\"editData('" + rpid + "')\">编辑</a>";
            }
            if ("1" == eval.ToString())
            {
                return "<span class=\"label label-success\">已送审</span>";
            }
            return "错误";
        }
