<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AddInfo.aspx.cs" Inherits="ECommerce.Web.Manage.Systems.Ajax_TempInfo.AddInfo" %>
...
<body>
    ...
    <form id="form1" runat="server" style="margin: 0" target="ifrSub">
        <div class="form-horizontal">
            <div class="modal-body">
                        ...
                        <table id="tabUser" width="100%" border="0" cellspacing="0">
                            <tr>
                                ...
                                <td style="text-align: right; width: 10px">
                                    <input type="checkbox" class="a-lab" name="cboProType" id='cboProType<%= dt.Tables[0].Rows[i]["ALID"] %>' value="<%= dt.Tables[0].Rows[i]["ALID"] %>" <%--<%= selectCheck(brandId.ToString(),dt.Tables[0].Rows[i]["PTId"].ToString() )%>--%>>
                                </td>
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input id="rboDouble" type="radio" runat="server" checked="True" name="rboSelectType" value="1" onclick="hideorg();" />
                        资讯
                        <input id="rboSingle" type="radio" runat="server" name="rboSelectType" value="2" onclick="shorg();" />
                        视频
                        <input id="Radio1" type="radio" runat="server" name="rboSelectType" value="3" onclick="hideorg();" />
                        专题
                    </div>
                </div>
                <div style="display: none">
                    <div style="margin-top: 5px;" id="ddladlink">
                        选择链接:
                    <asp:DropDownList runat="server" ID="ddlOrgName" CssClass="ddlinfo">
                    </asp:DropDownList>
                    </div>
                </div>
                <div id="dorg" style="display: none">
                    <div class="control-group">
                    ...
                    <div class="control-group">
                        <label class="control-label" for="inputPassword"><span style="color: red;">*</span>内容视频</label>
                        <div class="controls">
        ...
        <div class="modal-footer">
            <a onclick="goAddinfo();" class="btn btn-success">确定</a>
            <button class="btn" data-dismiss="modal" aria-hidden="true">关闭</button>
        </div>
        <script>
            $(document).ready(function () { window.top.uploader6.reset(); window.top.uploader.reset(); window.top.uploader2.reset(); window.top.uploader5.reset(); });
            function check(obj) {
                if (isNaN(obj.value)) {
                    alert("请输入数字");
                    obj.value = "";
